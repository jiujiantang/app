...
function protectorMouseDown(event) {
    ...
    if (event.button == 0 && !page.isMouseDown) {
      ...
      var protectorMouseUp = function (event) {
        ...
        //   $('#zh_drag_crop')[0].style.bottom = 'auto';
        //   $('#zh_drag_cancel')[0].style.bottom = 'auto';
        //   $('#zh_drag_crop')[0].style.top = '10px';
        //   $('#zh_drag_cancel')[0].style.top = '10px';
			...
			if (page.dragging || page.resizing) {
				...
				if (page.dragging || (page.resizing && ['tr', 'tl', 'br', 'bl'].indexOf(_direct) != -1)) {
					...
					if (_direct == 'mt' ) {
						...
						// _height = yPosition - page.startY;
						// _left = page.startX;
						// _top = page.startY - $('#zhuohu_collector_container').height();
						...
						// _height = Math.abs(_height);
						// page.endY = yPosition;
						// console.log('高度'+_height) endY
					} else if (_direct == 'mr' || _direct == 'ml') {
						// console.log(page)
						// _width = xPosition - page.startX;
						// _left = _width > 0 ? page.startX : xPosition;
						// _top = $('#zhuohu_collector_container')[0].offsetTop;
						// _width = Math.abs(_width);
						// _height = $('#zhuohu_collector_container').height();
						// page.endX = xPosition;
						_width = xPosition - page.startX;
						_left = _width > 0 ? page.startX : xPosition;
...
function SingleVisibleSizeFn(response) {
	// console.log(response)
	// console.log($('#zhuohu_collector_boundary'))
...
// 截屏完成
function onResponseVisibleSize(response) {
	// console.log(response)
    switch(response.msg) {
      case 'zhuohu_capture_viewport_selected':
        // 截图起始 x 坐标
		//  此处弃用
        // screenshot.startX = response.startX;
        screenshot.startX = response.startX;
    ...
    case 'scroll_init_done':
        ...
        setTimeout(function() {
            captureAndScroll(response.page_info);
        }, 100);
        break;
      default:
        break;
...
function CanvasGraph(data){
    ...
    image.onload = function() {
      ...
      var ext = image.src.substring(image.src.lastIndexOf(".") + 1).toLowerCase()
        ...
        // sendMessage({uploadPage:'canvasImgBase',format:format});;
        sendMessage({
          uploadPage: 'scroll_next',
          visibleWidth: visibleWidth,
          visibleHeight: visibleHeight
        }, onResponseVisibleSize);
